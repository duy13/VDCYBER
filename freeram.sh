...
run=`netstat -lntup|grep 8080`; if [ "$run" = "" ]; then service lsws restart; fi
...
(( RAMdangdung <= 100 )) && service lsws restart && service mysql restart && sh -c "sync; echo 3 > /proc/sys/vm/drop_caches"